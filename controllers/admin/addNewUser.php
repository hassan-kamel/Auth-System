...
if (checkRequestMethod($_SERVER['REQUEST_METHOD']) && checkPostInput('name')) {
    ...
    $_SESSION['addUserData'] = $data;

    if (!empty($errors)) {
        $_SESSION['addUserErrors'] = $errors;
        redirect(URL."pages/admin/addNewUser.php");
    }
    else{
        $data['password'] = sha1($password);
        $_SESSION['addUserData'] = $data;
        $new_user = $_SESSION['addUserData'];
        addNewUser($new_user['name'],$new_user['email'],$new_user['password'],$new_user['file']);
